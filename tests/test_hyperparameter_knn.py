...
from src.run_hyperparameter_knn import run_hyperparameter_knn
...
def test_knn_output_type():
    output_knn = run_hyperparameter_knn(train_x, train_y, param_grid, preprocessor)
...
def test_knn_score_value():
    result = run_hyperparameter_knn(train_x, train_y, param_grid, preprocessor)
...
def test_knn_correct_columns():
    ...
    assert all(col in run_hyperparameter_knn(train_x, train_y, param_grid, preprocessor).columns for col in expected_columns)


#Test whether all the values in the param_grid appear in the data frame
def test_knn_neighbor():
    result_knn = run_hyperparameter_knn(train_x, train_y, param_grid, preprocessor)
...
def test_knn_missing__error():
    with pytest.raises(TypeError) as custom_string:
        run_hyperparameter_knn(train_y, param_grid=param_grid, preprocessor=preprocessor)
    assert str(custom_string.value), "X_train is required as an additional argument"

    with pytest.raises(TypeError) as custom_string:
        run_hyperparameter_knn(train_x, param_grid=param_grid, preprocessor=preprocessor)
    assert str(custom_string.value), "y_train is required as an additional argument"

    with pytest.raises(KeyError) as custom_string:
        run_hyperparameter_knn(train_x, train_y, preprocessor=preprocessor)
...
def test_knn_type__error_order():
    with pytest.raises(TypeError) as custom_string:
        run_hyperparameter_knn(train_y, train_x, param_grid, preprocessor)
    assert str(custom_string.value), "y_train needs to be 1-dimensional array-like object"

    with pytest.raises(TypeError) as custom_string:
        run_hyperparameter_knn(train_x, train_y, preprocessor, param_grid)
